...
export function generateStaticParams() {
  return getGalleryCategories("clients").map((category) => ({
    slug: category.slug,
  }));
}
...
async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const category = getGalleryCategory("clients", slug);
    ...
    <>
      ...
      <div className="min-h-screen bg-obsidian pt-32 text-linen md:pt-40">
        ...
        <div className="px-5 pb-8 md:px-10">
          <p className="font-body text-[10px] uppercase tracking-mega text-flare">/ Client</p>
